...
Mattermost group-like command 必须提到当前 bot，例如 `!status @your_bot_username`、`!auto high @your_bot_username`、`@your_bot_username !status` 或 `!status@your_bot_username`。使用相同 target 形式指向其他 bot 的 command 会被忽略，包括 `@other_bot !status` 和 `@other_bot /status`。
