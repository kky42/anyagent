...
`/schedule add` 和 `!schedule add` 使用三行或更多行：第一行是 `schedule add <heartbeat|background> <name>`，第二行是五字段 cron 表达式，后续行是 prompt。`heartbeat` schedule 会在当前聊天 session 中排入一个普通 turn。`background` schedule 会启动一个新的 agent turn，并在完成后发送带标记的通知。
