...
| `/reset` | `!reset` | 为当前聊天重新加载 Agent Profile 默认值和已保存的 schedule，并清除当前聊天的覆盖配置。 |
| `/clear_cache` | `!clear_cache` | 删除当前聊天的附件缓存。 |
| `/schedule` | `!schedule` | 查看或管理当前聊天的定时运行。 |

聊天里的 `/reset` / `!reset` 只重新加载当前 Agent Profile 默认值，不会重新加载或修改 bot username、token、server URL、manager list 等聊天绑定字段。
...
`anyagent reset --agent <id>` 会执行 Agent Profile Reset：在运行中的 relay 里重新加载这个 profile，并协调它的聊天绑定，包括新增、删除、变更和迁移到该 profile 的绑定。它会 reset 该 profile 历史关联的 live conversation 和 durable-only conversation，保留 schedule 定义，中止正在运行的前台和后台任务，并为仍然 active 的绑定重新同步 timer。

带 conversation selector 的形式等价于在该 conversation 里发送 `/reset` 或 `!reset`。它必须提供完整 selector：agent、platform、binding 和 conversation id。

CLI reset 只支持 online 模式。relay 会启动本地 loopback control endpoint，并在 `~/.anyagent/run/` 下写入按 config 区分的 control file；CLI 会自动从该文件读取本地 bearer token。用户不需要手动传 token，也不会回退到离线修改状态文件。
...
需要一次性应用所有全局配置变更时，重启 relay 进程。进程重启会从磁盘重新加载所有 agent profile 和聊天绑定；新增 agent 会开始运行，删除的 agent 会停止，token、workdir、manager、model 或权限默认值等变更也会生效。对于单个运行中的 profile，可以使用 `anyagent reset --agent <id>` 在不重启整个 relay 的情况下应用这些绑定和 profile 变更。聊天里的 `/reset` 只影响当前 chat session，不是全局 relay reload。
